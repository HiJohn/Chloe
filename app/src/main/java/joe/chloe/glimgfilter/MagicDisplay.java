...
public abstract class MagicDisplay implements Renderer {
     ...
     */
    protected GlFilter mFilters;

    protected FilterType currentFilterType = FilterType.DEFAULT;
...
//    private MagicFilterAdjuster mFilterAdjust;
    ...
    public MagicDisplay(Context context, GLSurfaceView glSurfaceView) {
        ...
        mFilters = FilterType.createGlFilter(FilterType.DEFAULT, context);
//        mFilterAdjust = new MagicFilterAdjuster(mFilters);
     ...
     * 设置滤镜
     */
    public void setFilter(final FilterType filterType) {
        ...
        mGLSurfaceView.queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                mFilters = null;
                mFilters = FilterType.createGlFilter(filterType, mContext);
                if (mFilters != null){
                    mFilters.setup();
                }
                onFilterChanged();
//                mFilterAdjust = new MagicFilterAdjuster(mFilters);
    ...
    protected void onFilterChanged() {
        if (mFilters == null)
            return;
        mFilters.setFrameSize(mSurfaceWidth, mSurfaceHeight);
...
//                mFilters.onOutputSizeChanged(width, height);
                mFilters.setFrameSize(mImageWidth, mImageHeight);
                int textureId = -1;
                if (newTexture) {
